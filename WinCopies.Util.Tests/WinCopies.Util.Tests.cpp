...
namespace WinCopies
{
		namespace Tests
		{
			void AssertResultIsTrue()
			{
				Assert::IsTrue(SomeDelegate::result, L"SomeDelegate::Action did not update the result variable.");

				SomeDelegate::result = false;
			}

			void AssertFuncDelegateResultIsTrue(IFuncDelegate<bool>* delegate)
			{
				bool result = false;
			...
			TEST_CLASS(FuncDelegateTests)
			{
			public:
				void AssertParameterized(IParameterizedFuncDelegate<int, bool>* delegate, bool* emptyArrayFoundPtr, Queue<int>** queuePtr)
				{
					AssertFuncDelegateResultIsTrue(delegate);

					Assert::IsTrue(*emptyArrayFoundPtr, L"SomeParameterizedDelegate::Action() not found empty array. Maybe not be reached?");
				...
				TEST_METHOD(FuncDelegate)
				{
					IFuncDelegate<bool>* delegate = new SomeFuncDelegate();
